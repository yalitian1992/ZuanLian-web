...
// }
/*var diamonds_list = 1;
var diamonds_timer = setInterval(function(){
    get_diamonds_list()
    $(function(){
        $(".diamonds-rotate").css("background","url(images/diamonds/"+ diamonds_list +".png)")
    })
},80)
...
// 资源加载函数
    var imgUrls = [
        ...
        {name: "pic_57", path:"images/diamonds/58.png"},
        {name: "pic_57", path:"images/diamonds/59.png"},
        {name: "pic_57", path:"images/diamonds/60.png"},
        {name: "pic_57", path:"images/diamonds/61.png"}
    ];
    ...
    function loadImages(imgUrls, callback) {
        ...
        for (var i = 0; i < imgUrls.length; i++) {
            ...
            imgEls[name] = imgEl;
            imgEl.addEventListener('load', function (event) { // 监听load事件
    ...
    loadImages(imgUrls,function(imgEls){
        ...
        var diamonds_timer = setInterval(function(){
            get_diamonds_list()
            $(function(){
                // $(".diamonds-rotate").css("background","url("+imgUrls[diamonds_list].path+")");
                // $(".diamonds-rotate").append(imgEls[("pic_" + diamonds_list)]);
                // $(".diamonds-rotate").append(imgEls[("pic_" + diamonds_list)]);
                $(".diamonds-rotate .img-nomal").attr("src", "images/diamonds/" + diamonds_list + ".png");
            })
        },50)
...
/*$(function(){
    $('#dowebok').fullpage({
        afterLoad: function(anchorLink, index){
            if(index == 1){
                $(".section:nth-child(1) .content h3")[0].animate({
                        "padding-top":"100px",
                    },1000);
            }
        },
